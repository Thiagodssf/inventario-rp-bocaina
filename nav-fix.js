...
function ensureAnivStructure(){
  ...
  if(!document.getElementById('aniversariantes')){
...
function loadAniv(){
  ...
  s.src='aniversariantes-v2.js?v=4';
  s.async=false;
...
function activate(view){
  if(view==='agenda'){
    ...
    if(!agenda){console.error('Agenda Administrativa: seção #agenda não encontrada');return;}
...
function bind(){
  ...
  document.querySelectorAll('.nav[data-view]').forEach(b=>{
    ...
    b.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();activate(b.dataset.view)};
  });
  document.querySelectorAll('#locationNav .loc-btn').forEach(b=>{
    if(b.dataset.navBound==='1')return;
    b.dataset.navBound='1';
    b.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();activate('inventory');const f=document.getElementById('locationFilter');if(f){f.value=b.dataset.loc||'';f.dispatchEvent(new Event('change',{bubbles:true}));}};
  });
  const menu=document.getElementById('mobileMenu');
  if(menu&&!menu.dataset.navBound){menu.dataset.navBound='1';menu.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();document.getElementById('sidebar')?.classList.toggle('open');};}
}
function agendaDelegation(e){
  const b=e.target.closest?.('.nav[data-view="agenda"]');
  if(!b)return;
  e.preventDefault();
...
setTimeout(bind,500);
document.addEventListener('click',agendaDelegation,true);
})();
