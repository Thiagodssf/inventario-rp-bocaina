...
function sync(){
 ...
 const foods=BANK.map(([name])=>map.get(norm(name))).filter(Boolean);
 const extras=[...map.values()].filter(x=>!BANK.some(b=>norm(b[0])===norm(x.name||'')));
 const merged=[...foods,...extras];write('bocainaFoods',merged);write('mun_foods',merged.map(x=>({...x,current_qty:Number(x.qty||0),min_qty:Number(x.min||0)})));
 localStorage.setItem('mun_bank_master_version','1');localStorage.setItem('mun_bank_master_at',new Date().toISOString());
 try{window.dispatchEvent(new CustomEvent('municiamento:bank-updated',{detail:{count:foods.length}}));}catch{}
}
window.bocainaGenreBank=BANK.map(x=>({name:x[0],unit:x[1],cost:x[2]}));
window.municiamentoSyncBank=sync;
