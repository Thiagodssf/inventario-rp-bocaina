...
function payment(r){
 ...
 const sentYes=sent==='true'||sent==='sim'||sent.startsWith('sim ')||sent.includes('enviado para pagamento')||sent.includes('enviada para pagamento');
 if(s.includes('enviado para pagamento')||s.includes('enviada para pagamento')||sentYes)return'enviado';
 return'pendente';
}
function fix(){
 ...
 body.querySelectorAll('tr.ne-row').forEach(row=>{
  ...
  if(payBadge.textContent.trim()!==payText)payBadge.textContent=payText;if(payBadge.className!==payClass)payBadge.className=payClass;if(sitBadge.textContent.trim()!==sitText)sitBadge.textContent=sitText;if(sitBadge.className!==sitClass)sitBadge.className=sitClass;
 });
}
let timer=0,observer=null;function schedule(){clearTimeout(timer);timer=setTimeout(fix,0)}
...
document.addEventListener('bocaina:remote-sync',schedule);window.addEventListener('storage',e=>{if(e.key===KEY)schedule()});
})();