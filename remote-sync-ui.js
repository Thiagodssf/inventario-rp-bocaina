...
function payment(r){
 ...
 if(p==='enviado'||p.includes('enviado')||p.includes('enviada')||s.includes('enviado para pagamento')||s.includes('enviada para pagamento')||sentYes)return'enviado';
 return'pendente';
}
function fix(){
 ...
 body.querySelectorAll('tr.ne-row').forEach(row=>{
  ...
  const i=Number(row.dataset.i),r=data[i];
  if(!r)return;
  const state=payment(r);
  const payBadge=c[6].querySelector('.ne-badge');
  const sitBadge=c[7].querySelector('.ne-badge');
  if(!payBadge||!sitBadge)return;
  let payText='Pendente',payClass='ne-badge pendente';
  if(state==='pago'){payText='Pago';payClass='ne-badge pago'}
  else if(state==='enviado'){payText='Enviado para pagamento';payClass='ne-badge envio'}
